...
\section{The \scs{Foot-Strut} Split}	 \label{sec:FSSplit}
...
%There is no reference in the literature (see chapter \onlyinsubfile{3}\notinsubfile{\ref{ch:LitReviewSocio}}) that the split in the South East is affected by social class \todocontent{does this matter or am I actually just interested in what split the CoRP-SE speakers have to compare to?} and so for purposes of comparison the CoRP-SE speakers are assumed to have the prototypical southern \FS{} split.

\subsection{The \scs{Foot-Strut} Split in CoRP-SE speakers}
...
\todocontent{find literature on strut vowel frontness}
...
A further model was run on the same speakers including \strutt{}, \scs{thought}, and schwa, to check the frontness of the \strutt{} vowel of in comparison to other vowels that would be found at a similar height in the vowel space; the model summary can be found in appendix \ref{app:STSEF2}. The comparison shows that the \strutt{} vowel in these speakers is significantly further forward than the \scs{thought} vowel (-399Hz, t=-13.50) and also significantly further back than the schwa (305Hz, t=8.16), placing it between the two in the vowel space, but closer to schwa (see figure \ref{fig:STF2SE}) than \scs{thought}.
`
\begin{figure}[h]
...
\subsection{The \scs{Foot-Strut} Split in DECTE speakers}
...
\subsection{The \scs{Foot-Strut} Split in CoRP-NE speakers} \label{subsec:FSNE}
...
\begin{figure}[h]
	\includesvg[width=\textwidth]{../figures/FS-NE-F2.svg}
...
In the CoRP-SE speakers, the \foot{} and \strutt{} vowel are distinguished in F1 (a difference of +110Hz) and F2 (-86Hz). Since the scales of F1 and F2 are different a difference of 110 is far larger within F1 than 86 is within F2. The F1 difference is clearly meaningful but while the F2 difference is present and statistically significant it needs to be considered more carefully because it is only a small difference within the acoustic vowel space and may not be phonologically meaningful\todowording{check}. Overall the \FS split can be described as mostly characterised by height and slightly by frontness. It was also found that the frontness of the \strutt{} vowel in the South East is between the schwa and \scs{thought} vowels, but closer to schwa. If the CoRP-NE speakers were behaving like the CoRP-SE speakers, we would expect them to have a similar pattern in both F1 and possibly F2.

In the DECTE speakers we see what can be considered \quotesingle{typical} North East \FS{} positions of \foot{} and \strutt{}. The speakers show no significant difference between \foot{} and \strutt{} in F1, and in F2 a small difference in mainly older speakers (+147Hz). It can be concluded that there is not a robust \FS{} split in the North East as found in the CoRP-SE speakers.
...
\subsection{F1 of \scs{Strut} only}
...
\begin{table}[htbp]
	...
	\begin{tabular}{lrrr}
		...
		\hline
	\end{tabular}%
	\caption{table showing effects of corpus group and speaker sex on F1 of \strutt{} (\todo{confirm coefficient sum vs effect size}calculated from coefficients in table \ref{tbl:strutF1})}
...
\subsection{F2 of \scs{Strut} only}
